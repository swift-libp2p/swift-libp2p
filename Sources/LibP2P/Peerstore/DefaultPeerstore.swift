...
internal final class BasicInMemoryPeerStore: PeerStore, @unchecked Sendable {
    ...
    func prunePeers(olderThan expiration: TimeAmount = .seconds(30)) -> EventLoopFuture<Void> {
        eventLoop.submit {
            guard self.isPruning == false else {
                self.logger.info("Pruning in progress, skipping...")
                return
            }
            self.isPruning = true
    ...
    private func _prunePeers(oldestPercent percent: Double) -> EventLoopFuture<Void> {
        self.eventLoop.next().submit {
            ...
            if oldestPrunablePeers.count < numberOfPeersToPrune {
                ...
                self.logger.warning(
                    "Found \(oldestPrunablePeers.count) of desired \(numberOfPeersToPrune) of a total \(self.store.count) peers"
                )
            }
    ...
    private func oldestFirst(
        _ lhs: Dictionary<String, ComprehensivePeer>.Element,
        _ rhs: Dictionary<String, ComprehensivePeer>.Element
    ) -> Bool {
    ...
    private func prunability(peer: ComprehensivePeer) -> MetadataBook.PrunableMetadata.Prunable {
        ...
        guard
            let prunableState = try? JSONDecoder().decode(MetadataBook.PrunableMetadata.self, from: Data(prunableData))
    ...
    func getKey(forPeer id: String, on: EventLoop? = nil) -> EventLoopFuture<PeerID> {
        eventLoop.submit { () -> PeerID in
            if let pid = self.store[id] { return pid.id } else { throw Errors.peerNotFound }
        }.hop(to: on ?? eventLoop)
    }
    ...
    func getMostRecentRecord(forPeer peer: PeerID, on: EventLoop? = nil) -> EventLoopFuture<PeerRecord?> {
        getPeer(withID: peer.b58String).map { compPeer in
            compPeer.records.max(by: { a, b in
