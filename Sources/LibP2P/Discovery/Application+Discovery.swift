...
//        }
        ...
        public func use<D:Discovery>(_ makeService: @escaping (Application) -> (D)) {
            if self.storage.discoveryServices[D.key] != nil { fatalError("DiscoveryService `\(D.key)` Already Installed") }
...
extension Application.DiscoveryServices {
    ...
    public func announce(_ service:ServiceRegistration) -> EventLoopFuture<TimeAmount> {
        guard case .service(let proto) = service else { return application.eventLoopGroup.any().makeFailedFuture(Errors.notYetImplemented) }
    ...
    public func onPeerDiscovered(_ register:AnyObject, closure: @escaping (PeerInfo) -> ()) {
