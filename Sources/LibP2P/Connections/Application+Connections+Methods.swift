//
//  Application+Connections+Methods.swift
//  
//
...
extension Application {
    ...
    func streamCountToPeer(_ ma:Multiaddr) -> EventLoopFuture<Int> {
        return self.connections.getConnectionsTo(ma, onlyMuxed: true, on: nil).map({ connections -> Int in
            var streamCount = 0
            connections.forEach { connection in
                streamCount += connection.streams.count
            }
            self.logger.info("There are \(streamCount) stream(s) across \(connections.count) connection(s) to peer \(ma.description)")
            return streamCount
        })
    }
